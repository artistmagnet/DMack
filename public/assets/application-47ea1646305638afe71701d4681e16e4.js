...
(function() {
  jQuery(function() {
    return $(".has-submenu").click(function(e) {
      return $(this).toggleClass("active");
    });
  });

  jQuery(function() {
    return $('#section_slot').sortable({
      axis: 'y',
...
(function() {
  var bindAjaxOption, selectChain, syncGet;
  ...
  jQuery(function() {
    ...
    return bindAjaxOption('#add-production', '#production_shows_attributes_0_venue_id', '#add-venue');
  });

  bindAjaxOption = function(origin_form_selector, select_selector, create_form_selector) {
    $(select_selector.concat(' + .extend-list')).click(function(e) {
      e.preventDefault();
      $(create_form_selector).show();
      $(create_form_selector + ' + .fade').height($(document).height()).show();
      return selectChain.push(create_form_selector);
    });
    $(document).bind("ajaxSuccess", create_form_selector, function(event, xhr, settings) {
      var $entity_form, $entity_form_frame, $error_container, $error_container_ul, $select, entId, entName;
      if (event.data === selectChain[selectChain.length - 1]) {
        ...
        if ($select.length) {
          ...
          return selectChain.pop();
        }
      }
    });
    return $(document).bind("ajaxError", create_form_selector, function(event, jqxhr, settings, exception) {
      var $entity_form, $error_container, $error_container_ul;
      $entity_form = $(event.data);
      if (event.data === selectChain[selectChain.length - 1]) {
        $error_container = $("#error_explanation", $entity_form);
        $error_container_ul = $("ul", $error_container);
  ...
  jQuery(function() {
    return $('#add-resume-role-link').click(function() {
      $('#add-resume-role + .fade').height($(document).height()).show();
      return $('#add-resume-role').show();
    });
  });

  jQuery(function() {
    $(document).bind("ajaxError", '#add-resume-role', function(event, jqxhr, settings, exception) {
      var $entity_form, $error_container, $error_container_ul;
      alert('not saved');
      console.log(event);
      console.log(jqxhr);
    ...
    return $(document).bind("ajaxSuccess", '#add-resume-role', function(event, xhr, settings) {
      var $entity_form, $entity_form_frame, $error_container, $error_container_ul;
      $entity_form = $(event.data);
      $entity_form_frame = $(event.data.concat(' + .fade'));
      $error_container = $("#error_explanation", $entity_form);
      $error_container_ul = $("ul", $error_container);
      ...
      $entity_form.hide();
      $entity_form_frame.hide();
      return syncGet($($entity_form).data("reload-url"));
    });
  });
...
(function() {
  ...
  AbstractChosen = (function() {
    ...
    AbstractChosen.prototype.results_option_build = function(options) {
      var content, data, link, _i, _j, _len, _len1, _ref, _ref1;
      content = '';
      if (this.get_search_text()) {
        _ref = this.options.some_results_links;
    ...
    AbstractChosen.prototype.result_add_optional_link = function(link) {
      return '<li class="no-results ' + link.classes + '"><a href="' + link.href + '">' + link.text + '</a></li>';
    };
    ...
    AbstractChosen.prototype.winnow_results = function() {
      ...
      if (results < 1 && searchText.length) {
        ...
        return this.no_results(searchText, this.options.no_results_links);
      } else {
...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.no_results = function(terms, links) {
      var link, link_html, no_results_html, _i, _len;
      if (links == null) {
        links = [];
      }
      no_results_html = $('<li class="no-results"> "<span></span>"' + this.results_none_found + '</li>');
      no_results_html.find("span").first().html(terms);
      this.search_results.append(no_results_html);
      for (_i = 0, _len = links.length; _i < _len; _i++) {
        link = links[_i];
        link_html = $('<li class="no-results ' + link.classes + '"><a href="' + link.href + '" class="add_new">' + link.text + '</a></li>');
        this.search_results.append(link_html);
      }
...
(function() {
  $(function() {
    $('.chosen-select').chosen({
      ...
      no_results_links: [
        {
          "text": "To add it as a new Production, click here",
          "classes": "add_prod",
          "href": "#"
        }, {
          "text": "To leave as text only, click here.",
          "classes": "add_text",
          "href": "#"
        }
      ],
    ...
    return $('.chosen-multiple-select').chosen({
      no_results_text: ' not found.',
      width: '382px',
